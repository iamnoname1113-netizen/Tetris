...
HEURISTIC_HARD = {
    # ── Standard features ─────────────────────────────────────────────────────
    # w_lines: raw line count reward kept small — multi-line bonus takes over
    "w_lines" :  2.0,
    # w_holes: simple hole count (kept as baseline penalty)
    "w_holes" : -15.0,
    # w_height: mild penalty — bot should build dense, not flat & low
    "w_height": -0.20,
    # w_bump: moderate penalty — wants flat surface but not at cost of lines
    "w_bump"  : -1.5,
    # w_tetris: legacy bonus (well exists) — superseded by w_well below
    "w_tetris":  0.0,

    # ── Hard-bot exclusive features (see heuristics.py) ───────────────────────
    # w_multi: PRIMARY driver — exponential reward for clearing 2/3/4 lines at once
    #   2 lines → +18    3 lines → +54    4 lines (Tetris) → +180
    "w_multi"  :  18.0,
    # w_near: reward rows that are 1 cell away from complete (set up future clears)
    "w_near"   :  2.5,
    # w_well: reward depth of the best Tetris well (encourages I-piece setups)
    "w_well"   :  3.5,
    # w_covered: heavy penalty for holes buried under many blocks (hard to fix)
    "w_covered": -3.5,
}
